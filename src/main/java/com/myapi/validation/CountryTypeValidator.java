...
public class CountryTypeValidator implements ConstraintValidator<CountryTypeChecker, String> {
  ...
  public boolean isValid(String country, ConstraintValidatorContext context) {
    ...
    if (StringUtils.isBlank(country)) {
      ValidatorUtils.customMessageForValidation(context, "The country is required");
      return false;
    }

    if (!acceptedValues.contains(country)) {
      ValidatorUtils.customMessageForValidation(context, "Invalid country, The country name must be in English and uppercase");
      return false;
    }
